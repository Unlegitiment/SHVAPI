...
void HC_example(HeistCelebHandle* inst) {
	...
	for (int i = 0; i < 3; i++) {
		...
		HC_ADD_COMPLETE_MESS_TO_WALL(h->scl[i].handle, h->wallID, "OHHH", "GARRY", "");
		HC_CREATE_STAT_TABLE(h->scl[i].handle, h->wallID, "table", 3);
		HC_ADD_STAT_TO_WALL(h->scl[i].handle, h->wallID, "table", "GARRY", "", FALSE, FALSE, "HUD_COLOUR_PLATINUM");
		HC_ADD_STAT_TO_WALL(h->scl[i].handle, h->wallID, "table", "HARRY", "", FALSE, FALSE, "HUD_COLOUR_SILVER");
		HC_ADD_STAT_TABLE_TO_WALL(h->scl[i].handle, h->wallID, "table");
		HC_ADD_COMPLETE_MESS_TO_WALL(h->scl[i].handle, h->wallID, "IM BOUTA", "CUUUUUUUUMMMMM", "");
		HC_PAUSE(h->scl[i].handle, h->wallID, 5 * 6);
		HC_ADD_JP(h->scl[i].handle, h->wallID, 15, FALSE);
		HC_ADD_RP_AND_BAR(h->scl[i].handle, h->wallID, 500, 1500, 16, 17, 1500, 2000, "NICE", "CUM!");
