...
CButtonUI::CButtonUI(CBox& __button__)
	: 
	m_Box(__button__),
	m_Buttons { 0 } //when we zero everything it has some adverse side effects
{
}
...
void CButtonUI::SetText(eText textToSet, CTextUI* textToInput)
{
	if (textToSet > eText::ETEXT_MAX - 1) textToSet = eText::ETEXT_LEFT;
	if (textToSet < eText::ETEXT_LEFT) textToSet = eText::ETEXT_LEFT;
	IdealDraw(textToSet, textToInput);
	this->m_Buttons[textToSet] = textToInput;
	return;
}
